import { FormFile, FormGraph, FormNode, FormRelation, ExecutionConfig } from '../types/FormTypes';
...
export class FormCompiler {
  ...
  compile(formFile: FormFile): FormGraph {
    ...
    const form = cloneDeep(formFile);
    
    // Build node map for O(1) lookups
    const nodes = new Map<string, FormNode>();
    form.nodes.forEach(node => {
      // Initialize default values
      if (!node.data.confidence) node.data.confidence = 1.0;
      if (!node.data.weight) node.data.weight = 1.0;
      if (!node.data.state) node.data.state = 'active';
      
      nodes.set(node.id, node);
    });

    // Build relation map for O(1) lookups
    const relations = new Map<string, FormRelation>();
    form.relations.forEach(relation => {
      // Initialize default values
      if (relation.strength === undefined) relation.strength = 1.0;
      if (relation.bidirectional === undefined) relation.bidirectional = false;
   ...
   */
  private buildAdjacencyLists(relations: FormRelation[]): {
   ...
   */
  private inferEntryPoints(nodes: FormNode[], reverseAdjacencyList: Map<string, string[]>): string[] {
   ...
   */
  private inferExitPoints(nodes: FormNode[], adjacencyList: Map<string, string[]>): string[] {
   ...
   */
  private calculateComplexity(adjacencyList: Map<string, string[]>): {
    maxDepth: number;
    avgBranching: number;
    cycleCount: number;
  } {
  ...
  private calculateMaxDepth(adjacencyList: Map<string, string[]>): number {
    ...
    const bfs = (startNode: string): number => {
      const queue: { nodeId: string; depth: number }[] = [{ nodeId: startNode, depth: 0 }];
      let localMaxDepth = 0;
   ...
   */
  private estimateCycleCount(adjacencyList: Map<string, string[]>): number {
    let cycleCount = 0;
    const visited = new Set<string>();
   ...
   */
  optimizeForExecution(graph: FormGraph, mode: 'speed' | 'memory' | 'balanced' = 'balanced'): FormGraph {
    const optimizedGraph = cloneDeep(graph);
   ...
   */
  private optimizeForSpeed(graph: FormGraph): void {
   ...
   */
  private optimizeForMemory(graph: FormGraph): void {
    // Remove optional fields with default values to save memory
    graph.nodes.forEach(node => {
      if (node.data.confidence === 1.0) delete node.data.confidence;
      if (node.data.weight === 1.0) delete node.data.weight;
      if (node.data.state === 'active') delete node.data.state;
    });

    graph.relations.forEach(relation => {
      if (relation.strength === 1.0) delete relation.strength;
      if (relation.bidirectional === false) delete relation.bidirectional;
    });
   ...
   */
  private optimizeBalanced(graph: FormGraph): void {
